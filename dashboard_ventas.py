...
with st.sidebar:
    ...
    # NUEVO: Uploader de archivo Excel
    uploaded_file = st.file_uploader("📂 Subir BD de Ventas (Excel)", type=["xlsx", "xls"])

# Si no hay archivo subido, mostramos un aviso y detenemos la ejecución del resto del código
if uploaded_file is None:
    st.info("👋 **¡Bienvenido al Dashboard!** Por favor, sube tu archivo Excel de ventas en el panel lateral izquierdo para comenzar el análisis.")
    st.stop()
...
@st.cache_data
def load_data(file):
    # En lugar del path directo, lee el archivo cargado en memoria
...
# Cargar la data pasando el archivo subido
...
#  SIDEBAR — FILTROS
# ─────────────────────────────────────────────
# Reabrimos el sidebar para mostrar los filtros dinámicamente ahora que el DF existe
with st.sidebar:
...
# 👇 👇 👇
# [A PARTIR DE AQUÍ MANTÉN TODO TU CÓDIGO ORIGINAL INTACTO]
# [Helpers, KPIs, Tabs y Gráficas...]
